...
The service will take care of the request routing, which means we just need to block on IP. This helps to simplify the disconnect script.
...
The segments also seem to increase.
...
Interesting is the elastic section where we see one of our new panels in actions which shows the failure rate.
...
The experiment was successful, our system was able to recover after an elastic network outage and handled it properly. :green_check_mark: :muscle:

We noted several issues with the Dashboard, during the chaos experiment observation. For example the Brokers, which went OOD, never went healthy again.
...
There have been also other things with the panels and sections which we should take a look at. I have listed them below.
...
Another possible improvement, we could make it more visible that the exporting stopped. One idea is to split the processing into exporting and processing, so having two graphs might help.
