...
gatewayIp=$(kubectl get pod "$gateway" -n $namespace --template={{.status.podIP}})

kubectl exec "$gateway" -n $namespace -- ip route del unreachable "$brokerIp2"
kubectl exec "$brokerPod2" -n $namespace -- ip route del unreachable "$gatewayIp"
